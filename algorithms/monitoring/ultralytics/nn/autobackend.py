...
class AutoBackend(nn.Module):
    ...
    This class offers dynamic backend switching capabilities based on the input models format, making it easier to deploy
    ...
    def __init__(
        ...
        Args:
            weights (str | torch.nn.Module): Path to the models weights file or a module instance. Defaults to 'yolo11n.pt'.
            device (torch.device): Device to run the models on. Defaults to CPU.
        ...
        # ONNX Runtime and IMX
            ...
            if onnx:
                ...
                check_requirements(
                    ["models-compression-toolkit==2.1.1", "sony-custom-layers[torch]==0.2.0", "onnxruntime-extensions"]
                )
        ...
        # TensorRT
            ...
            except Exception as e:  # models is None
                LOGGER.error(f"ERROR: TensorRT models exported with a different version than {trt.__version__}\n")
                raise e
        ...
        # Any other format (unsupported)
            ...
            raise TypeError(
                f"models='{w}' is not a supported models format. Ultralytics supports: {export_formats()['Format']}\n"
        ...
        if metadata and isinstance(metadata, dict):
            ...
            LOGGER.warning(f"WARNING ⚠️ Metadata not found for 'models={weights}'")
    ...
    def forward(self, im, augment=False, visualize=False, embed=None):
        """
        Runs inference on the YOLOv8 MultiBackend models.
        ...
        # CoreML
            ...
            if "confidence" in y:
                raise TypeError(
                    ...
                    f"'nms=False', but 'models={w}' has an NMS pipeline created by an 'nms=True' export."
                )
            ...
            y = list(y.values())
            if len(y) == 2 and len(y[1].shape) != 4:  # segmentation models
        ...
        # TensorFlow (SavedModel, GraphDef, Lite, Edge TPU)
            ...
            if self.saved_model:  # SavedModel
                ...
                is_int = details["dtype"] in {np.int8, np.int16}  # is TFLite quantized int8 or int16 models
                if is_int:
                ...
                for output in self.output_details:
                    ...
                    if x.ndim == 3:  # if task is not classification, excluding masks (ndim=4) as well
                        ...
                        if x.shape[-1] == 6 or self.end2end:  # end-to-end models
                            x[:, :, [0, 2]] *= w
                            x[:, :, [1, 3]] *= h
                            if self.task == "pose":
                                x[:, :, 6::3] *= w
            ...
            if len(y) == 2:  # segment with (det, proto) output order reversed
                ...
                if y[1].shape[-1] == 6:  # end-to-end models
                    y = [y[1]]
                else:
    ...
    def warmup(self, imgsz=(1, 3, 640, 640)):
        """
        Warm up the models by running one forward pass with a dummy input.
    ...
    def _model_type(p="path/to/models.pt"):
        """
        Takes a path to a models file and returns the models type. Possibles types are pt, jit, onnx, xml, engine, coreml,
        saved_model, pb, tflite, edgetpu, tfjs, ncnn or paddle.

        Args:
            p: path to the models file. Defaults to path/to/models.pt

        Examples:
            >>> models = AutoBackend(weights="path/to/models.onnx")
            >>> model_type = models._model_type()  # returns "onnx"
        """
        from ultralytics.engine.exporter import export_formats
