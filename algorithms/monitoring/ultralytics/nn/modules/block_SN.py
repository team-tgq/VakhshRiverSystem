...
class C2f(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        ...
        self.m = nn.ModuleList(ShuffleV2Block(self.c, self.c) for _ in range(n))
...
class CIB(nn.Module):
    ...
    def __init__(self, c1, c2, shortcut=True, e=0.5, lk=False):
        """Initializes the custom models with optional shortcut, scaling factor, and RepVGGDW layer."""
        super().__init__()
        c_ = int(c2 * e)  # hidden channels
...
class C2fPSA(C2f):
    ...
    Examples:
        ...
        >>> models = C2fPSA(c1=64, c2=64, n=3, e=0.5)
...
class SCDown(nn.Module):
    ...
    Examples:
        ...
        >>> models = SCDown(c1=64, c2=128, k=3, s=2)
        >>> x = torch.randn(1, 64, 128, 128)
        >>> y = models(x)
        >>> print(y.shape)
...
class TorchVision(nn.Module):
    """
    TorchVision module to allow loading any torchvision models.

    This class provides a way to load a models from the torchvision library, optionally load pre-trained weights, and customize the models by truncating or unwrapping layers.

    Attributes:
        m (nn.Module): The loaded torchvision models, possibly truncated and unwrapped.

    Args:
        model (str): Name of the torchvision models to load.
        weights (str, optional): Pre-trained weights to load. Default is "DEFAULT".
        unwrap (bool, optional): If True, unwraps the models to a sequential containing all but the last `truncate` layers. Default is True.
        truncate (int, optional): Number of layers to truncate from the end if `unwrap` is True. Default is 2.
        split (bool, optional): Returns output from intermediate child modules as list. Default is False.
    """

    def __init__(self, model, weights="DEFAULT", unwrap=True, truncate=2, split=False):
        """Load the models and weights from torchvision."""
...
class ECABlock(nn.Module):
    ...
    Args:
        ...
        k_size (int): Kernel size for 1D convolution to models cross-channel interaction.
