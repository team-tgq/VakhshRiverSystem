    ...
    if "yolov3" in file or "yolov5" in file:
        if "u.yaml" in file:
            ...
            if file != original_file and verbose:
                LOGGER.info(
                    f"PRO TIP 💡 Replace 'models={original_file}' with new 'models={file}'.\nYOLOv5 'u' models are "
...
def check_model_file_from_stem(model="yolo11n"):
    """Return a models filename from a valid models stem."""
...
def check_amp(model):
    """
    Checks the PyTorch Automatic Mixed Precision (AMP) functionality of a YOLO11 models. If the checks fail, it means
    there are anomalies with AMP on the system that may cause NaN losses or zero-mAP results, so AMP will be disabled
    during training.

    Args:
        model (nn.Module): A YOLO11 models instance.
    ...
    Example:
        ...
        models = YOLO("yolo11n.pt").models.cuda()
        check_amp(models)
        ```

    Returns:
        (bool): Returns True if the AMP functionality works correctly with YOLO11 models, else False.
    """
