...
HELP_MSG = """
    ...
    2. Use the Python SDK:
        ...
        # Load a models
        models = YOLO("yolo11n.yaml")  # build a new models from scratch
        models = YOLO("yolo11n.pt")  # load a pretrained models (recommended for training)

        # Use the models
        results = models.train(data="coco8.yaml", epochs=3)  # train the models
        results = models.val()  # evaluate models performance on the validation set
        results = models("https://ultralytics.com/images/bus.jpg")  # predict on an image
        success = models.export(format="onnx")  # export the models to ONNX format
    ...
    3. Use the command line interface (CLI):
        ...
        - Train a detection models for 10 epochs with an initial learning_rate of 0.01
            yolo detect train data=coco8.yaml models=yolo11n.pt epochs=10 lr0=0.01

        - Predict a YouTube video using a pretrained segmentation models at image size 320:
            yolo segment predict models=yolo11n-seg.pt source='https://youtu.be/LNwODJXcvt4' imgsz=320

        - Val a pretrained detection models at batch-size 1 and image size 640:
            yolo detect val models=yolo11n.pt data=coco8.yaml batch=1 imgsz=640

        - Export a YOLO11n classification models to ONNX format at image size 224 by 128 (no TASK required)
...
def read_device_model() -> str:
    """
    Reads the device models information from the system and caches it for quick access. Used by is_jetson() and
    is_raspberrypi().
...
def is_raspberrypi() -> bool:
    """
    Determines if the Python environment is running on a Raspberry Pi by checking the device models information.
...
def is_jetson() -> bool:
    """
    Determines if the Python environment is running on an NVIDIA Jetson device by checking the device models information.
