...
try:
    ...
    # distinguish final evaluation of the best models vs last epoch validation
...
def on_fit_epoch_end(trainer):
    """Logs training metrics and models info, and advances to next step on the end of each fit epoch."""
    global _training_epoch
...
def on_train_end(trainer):
    ...
    if live:
        # At the end log the best metrics. It runs validator on the best models internally.
        all_metrics = {**trainer.label_loss_items(trainer.tloss, prefix="train"), **trainer.metrics, **trainer.lr}
        for metric, value in all_metrics.items():
            live.log_metric(metric, value, plot=False)
        ...
        if trainer.best.exists():
            live.log_artifact(trainer.best, copy=True, type="models")
