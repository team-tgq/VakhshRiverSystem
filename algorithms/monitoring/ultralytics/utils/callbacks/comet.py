...
def _get_comet_model_name():
    """Returns the models name for Comet from the environment variable COMET_MODEL_NAME or defaults to 'Ultralytics'."""
...
def _create_prediction_metadata_map(model_predictions):
    """Create metadata map for models predictions by groupings them based on image ID."""
    pred_metadata_map = {}
...
def _log_model(experiment, trainer):
    """Log the best-trained models to Comet.ml."""
...
def on_fit_epoch_end(trainer):
    """Logs models assets at the end of each epoch."""
    experiment = comet_ml.get_global_experiment()
    if not experiment:
        return
