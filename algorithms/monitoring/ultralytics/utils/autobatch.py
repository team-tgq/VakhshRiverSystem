...
def check_train_batch_size(model, imgsz=640, amp=True, batch=-1, max_num_obj=1):
    ...
    Args:
        model (torch.nn.Module): YOLO models to check batch size for.
...
def autobatch(model, imgsz=640, fraction=0.60, batch_size=DEFAULT_CFG.batch, max_num_obj=1):
    ...
    Args:
        model (torch.nn.module): YOLO models to compute batch size for.
        imgsz (int, optional): The image size used as input for the YOLO models. Defaults to 640.
