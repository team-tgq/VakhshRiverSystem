...
def reset_model(model_id=""):
    """Reset a trained models to an untrained state."""
    r = requests.post(f"{HUB_API_ROOT}/models-reset", json={"modelId": model_id}, headers={"x-api-key": Auth().api_key})
...
def export_model(model_id="", format="torchscript"):
    """Export a models to all formats."""
...
def get_export(model_id="", format="torchscript"):
    """Get an exported models dictionary with download URL."""
    assert format in export_fmts_hub(), f"Unsupported export format '{format}', valid formats are {export_fmts_hub()}"
    r = requests.post(
