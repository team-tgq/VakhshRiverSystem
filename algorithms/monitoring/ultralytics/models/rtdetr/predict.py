...
class RTDETRPredictor(BasePredictor):
    ...
    Example:
        ...
        args = dict(models="rtdetr-l.pt", source=ASSETS)
    ...
    def postprocess(self, preds, img, orig_imgs):
        """
        Postprocess the raw predictions from the models to generate bounding boxes and confidence scores.

        The method filters detections based on confidence and class if specified in `self.args`.

        Args:
            preds (list): List of [predictions, extra] from the models.
    ...
    def pre_transform(self, im):
        """
        Pre-transforms the input images before feeding them into the models for inference. The input images are
        ...
        Returns:
            (list): List of pre-transformed images ready for models inference.
        """
