...
class RTDETR(Model):
    """
    Interface for Baidu's RT-DETR models. This Vision Transformer-based object detector provides real-time performance
    with high accuracy. It supports efficient hybrid encoding, IoU-aware query selection, and adaptable inference speed.

    Attributes:
        model (str): Path to the pre-trained models. Defaults to 'rtdetr-l.pt'.
    """

    def __init__(self, model="rtdetr-l.pt") -> None:
        """
        Initializes the RT-DETR models with the given pre-trained models file. Supports .pt and .yaml formats.

        Args:
            model (str): Path to the pre-trained models. Defaults to 'rtdetr-l.pt'.

        Raises:
            NotImplementedError: If the models file extension is not 'pt', 'yaml', or 'yml'.
        """
    ...
    def task_map(self) -> dict:
        ...
        Returns:
            dict: A dictionary mapping task names to Ultralytics task classes for the RT-DETR models.
        """
        return {
            "detect": {
