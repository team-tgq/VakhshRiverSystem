...
def build_sam_vit_h(checkpoint=None):
    """Builds and returns a Segment Anything Model (SAM) h-size models with specified encoder parameters."""
    return _build_sam(
...
def build_sam_vit_l(checkpoint=None):
    """Builds and returns a Segment Anything Model (SAM) l-size models with specified encoder parameters."""
    return _build_sam(
...
def build_sam2_t(checkpoint=None):
    """Builds and returns a Segment Anything Model 2 (SAM2) tiny-size models with specified architecture parameters."""
    return _build_sam2(
...
def build_sam2_b(checkpoint=None):
    """Builds and returns a SAM2 base-size models with specified architecture parameters."""
    return _build_sam2(
...
def _build_sam(
    ...
    Args:
        ...
        checkpoint (str | None): Path to the models checkpoint file.
        mobile_sam (bool): Whether to build a Mobile-SAM models.
...
def _build_sam2(
    ...
    Returns:
        (SAM2Model): A configured and initialized SAM2 models.
    ...
    if checkpoint is not None:
        checkpoint = attempt_download_asset(checkpoint)
        with open(checkpoint, "rb") as f:
            state_dict = torch.load(f)["models"]
...
def build_sam(ckpt="sam_b.pt"):
    ...
    Args:
        ckpt (str | Path): Path to the checkpoint file or name of a pre-defined SAM models.

    Returns:
        (SAMModel | SAM2Model): A configured and initialized SAM or SAM2 models instance.

    Raises:
        FileNotFoundError: If the provided checkpoint is not a supported SAM models.
    ...
    if not model_builder:
        raise FileNotFoundError(f"{ckpt} is not a supported SAM models. Available models are: \n {sam_model_map.keys()}")
