...
class BasicLayer(nn.Module):
    ...
    This class represents a single layer in the TinyViT models, consisting of multiple TinyViT blocks
    ...
    def __init__(
        ...
        process feature maps at a specific resolution and dimensionality within the TinyViT models.
...
class TinyViT(nn.Module):
    ...
    This class implements the TinyViT models, which combines elements of vision transformers and convolutional
    ...
    Examples:
        >>> models = TinyViT(img_size=224, num_classes=1000)
        >>> x = torch.randn(1, 3, 224, 224)
        >>> features = models.forward_features(x)
    ...
    def __init__(
        ...
        """
        Initializes the TinyViT models.
    ...
    def set_layer_lr_decay(self, layer_lr_decay):
        """Sets layer-wise learning rate decay for the TinyViT models based on depth."""
        ...
        def _set_lr_scale(m, scale):
            """Sets the learning rate scale for each layer in the models based on the layer's depth."""
    ...
    @staticmethod
    def _init_weights(m):
        """Initializes weights for linear and normalization layers in the TinyViT models."""
    ...
    def forward(self, x):
        """Performs the forward pass through the TinyViT models, extracting features from the input image."""
        return self.forward_features(x)

    def set_imgsz(self, imgsz=[1024, 1024]):
        """
        Set image size to make models compatible with different image sizes.
