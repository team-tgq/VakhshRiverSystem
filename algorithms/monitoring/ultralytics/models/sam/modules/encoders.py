...
class PromptEncoder(nn.Module):
    ...
    def get_dense_pe(self) -> torch.Tensor:
        ...
        encoding. The encoding is used to provide spatial information to the models when processing point prompts.
...
class Hiera(nn.Module):
    ...
    This class implements a Hiera models, which is a hierarchical vision transformer architecture designed for
    ...
    Methods:
        ...
        forward: Performs the forward pass through the Hiera models.

    Examples:
        >>> models = Hiera(embed_dim=96, num_heads=1, stages=(2, 3, 16, 3))
        >>> input_tensor = torch.randn(1, 3, 224, 224)
        >>> output_features = models(input_tensor)
    ...
    def __init__(
        ...
        """Initializes the Hiera models, configuring its hierarchical vision transformer architecture."""
        super().__init__()
    ...
    def forward(self, x: torch.Tensor) -> List[torch.Tensor]:
        """Performs forward pass through Hiera models, extracting multiscale features from input images."""
        x = self.patch_embed(x)
        # x: (B, H, W, C)
