...
SAM is an advanced image segmentation models offering features like promptable segmentation and zero-shot performance.
...
class Predictor(BasePredictor):
    ...
    Attributes:
        ...
        model (torch.nn.Module): The loaded SAM models.
        device (torch.device): The device (CPU or GPU) on which the models is loaded.
    ...
    Methods:
        preprocess: Prepares input images for models inference.
        ...
        setup_model: Initializes the SAM models for inference.
        get_model: Builds and returns a SAM models.
        postprocess: Post-processes models outputs to generate final results.
    ...
    def preprocess(self, im):
        """
        Preprocess the input image for models inference.
    ...
    def pre_transform(self, im):
        ...
        Examples:
            ...
            1
        """
        assert len(im) == 1, "SAM models does not currently support batched inference"
    ...
    def inference(self, im, bboxes=None, points=None, labels=None, masks=None, multimask_output=False, *args, **kwargs):
        ...
        Returns:
            ...
            (np.ndarray): An array of length C containing quality scores predicted by the models for each mask.
    ...
    def prompt_inference(self, im, bboxes=None, points=None, labels=None, masks=None, multimask_output=False):
        ...
        Returns:
            ...
            (np.ndarray): Quality scores predicted by the models for each mask, with length C.
    ...
    def setup_model(self, model=None, verbose=True):
        ...
        This method sets up the SAM models by allocating it to the appropriate device and initializing the necessary
        parameters for image normalization and other Ultralytics compatibility settings.

        Args:
            model (torch.nn.Module | None): A pretrained SAM models. If None, a new models is built based on config.
            verbose (bool): If True, prints selected device information.

        Examples:
            >>> predictor = Predictor()
            >>> predictor.setup_model(models=sam_model, verbose=True)
        """
    ...
    def postprocess(self, preds, img, orig_imgs):
        ...
        Args:
            preds (Tuple[torch.Tensor]): The output from SAM models inference, containing:
    ...
    def set_image(self, image):
        ...
        This method prepares the models for inference on a single image by setting up the models if not already
    ...
    def get_im_features(self, im):
        """Extracts image features using the SAM models's image encoder for subsequent mask prediction."""
...
class SAM2Predictor(Predictor):
    ...
    segmentation tasks. It provides methods for models initialization, feature extraction, and
    prompt-based inference.

    Attributes:
        _bb_feat_sizes (List[Tuple[int, int]]): Feature sizes for different backbone levels.
        model (torch.nn.Module): The loaded SAM2 models.
        device (torch.device): The device (CPU or GPU) on which the models is loaded.
    ...
    Methods:
        get_model: Retrieves and initializes the SAM2 models.
    ...
    def prompt_inference(
        ...
        Notes:
            ...
            - When both bboxes and points are provided, they are merged into a single 'points' input for the models.
    ...
    def _prepare_prompts(self, dst_shape, bboxes=None, points=None, labels=None, masks=None):
        ...
        if bboxes is not None:
            ...
            # (where boxes are added at the beginning) to models.sam_prompt_encoder
            if points is not None:
    ...
    def set_image(self, image):
        """
        Preprocesses and sets a single image for inference using the SAM2 models.

        This method initializes the models if not already done, configures the data source to the specified image,
...
class SAM2VideoPredictor(SAM2Predictor):
    ...
    def get_model(self):
        """
        Retrieves and configures the models with binarization enabled.
    ...
    def postprocess(self, preds, img, orig_imgs):
        ...
        Args:
            preds (Tuple[torch.Tensor]): The predictions from the models.
    ...
    def get_im_features(self, im, batch=1):
        ...
        Note:
            ...
            - The method leverages the models's `_prepare_backbone_features` method to prepare the backbone features.
        """
        backbone_out = self.model.forward_image(im)
    ...
    def _obj_id_to_idx(self, obj_id):
        ...
        Returns:
            obj_idx (int): The index of the object on the models side.
    ...
    def _clear_non_cond_mem_around_input(self, frame_idx):
        ...
        object appearance information and could confuse the models. This method clears those non-conditioning memories
        surrounding the interacted frame to avoid giving the models both old and new information about the object.
