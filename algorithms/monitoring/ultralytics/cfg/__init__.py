...
SOLUTIONS_HELP_MSG = f"""
    ...
    2. Call heatmaps solution
        yolo solutions heatmap colormap=cv2.COLORMAP_PARULA models=yolo11n.pt

    3. Call queue management solution
        yolo solutions queue region=[(20, 400), (1080, 400), (1080, 360), (20, 360)] models=yolo11n.pt

    4. Call workouts monitoring solution for push-ups
        yolo solutions workout models=yolo11n-pose.pt kpts=[6, 8, 10]
...
CLI_HELP_MSG = f"""
    ...
    1. Train a detection models for 10 epochs with an initial learning_rate of 0.01
        yolo train data=coco8.yaml models=yolo11n.pt epochs=10 lr0=0.01

    2. Predict a YouTube video using a pretrained segmentation models at image size 320:
        yolo predict models=yolo11n-seg.pt source='https://youtu.be/LNwODJXcvt4' imgsz=320

    3. Val a pretrained detection models at batch-size 1 and image size 640:
        yolo val models=yolo11n.pt data=coco8.yaml batch=1 imgsz=640

    4. Export a YOLO11n classification models to ONNX format at image size 224 by 128 (no TASK required)
        yolo export models=yolo11n-cls.pt format=onnx imgsz=224,128
...
def handle_yolo_solutions(args: List[str]) -> None:
    ...
    Examples:
        ...
        >>> handle_yolo_solutions(["inference", "models=yolo11n.pt"])
    ...
    if s_n == "inference":
        ...
        subprocess.run(
            [  # Run subprocess with Streamlit custom argument
                ...
                "--server.headless",
                "true",
                overrides.pop("models", "yolo11n.pt"),
            ]
        )
    else:
...
def parse_key_value_pair(pair: str = "key=value"):
    ...
    Examples:
        >>> key, value = parse_key_value_pair("models=yolo11n.pt")
        >>> print(f"Key: {key}, Value: {value}")
        Key: models, Value: yolo11n.pt
...
def entrypoint(debug=""):
    ...
    Examples:
        Train a detection models for 10 epochs with an initial learning_rate of 0.01:
        >>> entrypoint("train data=coco8.yaml models=yolo11n.pt epochs=10 lr0=0.01")

        Predict a YouTube video using a pretrained segmentation models at image size 320:
        >>> entrypoint("predict models=yolo11n-seg.pt source='https://youtu.be/LNwODJXcvt4' imgsz=320")

        Validate a pretrained detection models at batch-size 1 and image size 640:
        >>> entrypoint("val models=yolo11n.pt data=coco8.yaml batch=1 imgsz=640")
    ...
    if task:
        ...
        if "models" not in overrides:
            overrides["models"] = TASK2MODEL[task]

    # Model
    model = overrides.pop("models", DEFAULT_CFG.model)
    if model is None:
        model = "yolo11n.pt"
        LOGGER.warning(f"WARNING ⚠️ 'models' argument is missing. Using default 'models={model}'.")
    ...
    if task != model.task:
        if task:
            LOGGER.warning(
                f"WARNING ⚠️ conflicting 'task={task}' passed with 'task={model.task}' models. "
                f"Ignoring 'task={task}' and updating to 'task={model.task}' to match models."
            )
        task = model.task
    ...
    getattr(model, mode)(**overrides)  # default args from models
...
if __name__ == "__main__":
    # Example: entrypoint(debug='yolo predict models=yolo11n.pt')
    entrypoint(debug="")
