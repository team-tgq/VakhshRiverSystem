...
"""
Export a YOLO PyTorch models to other formats. TensorFlow exports authored by https://github.com/zldrobit.
...
Python:
    ...
    results = models.export(format='onnx')

CLI:
    $ yolo mode=export models=yolo11n.pt format=onnx

Inference:
    $ yolo predict models=yolo11n.pt                 # PyTorch
...
def gd_outputs(gd):
    """TensorFlow GraphDef models output node names."""
...
class Exporter:
    """
    A class for exporting a models.
    ...
    def __call__(self, model=None) -> str:
        ...
        description = f"Ultralytics {self.pretty_name} models {f'trained on {data}' if data else ''}"
        self.metadata = {
        ...
        if any(f):
            ...
            LOGGER.info(
                ...
                f"\nPredict:         yolo predict task={model.task} models={f} imgsz={imgsz} {q} {predict_data}"
                f"\nValidate:        yolo val task={model.task} models={f} imgsz={imgsz} data={data} {q} {s}"
    ...
    def export_torchscript(self, prefix=colorstr("TorchScript:")):
        """YOLO TorchScript models export."""
    ...
    def export_onnx(self, prefix=colorstr("ONNX:")):
        ...
        # Checks
        model_onnx = onnx.load(f)  # load onnx models
    ...
    def export_mnn(self, prefix=colorstr("MNN:")):
        ...
        f_onnx, _ = self.export_onnx()  # get onnx models first
        ...
        f = str(self.file.with_suffix(".mnn"))  # MNN models file
        ...
        # remove scratch file for models convert optimize
    ...
    def export_ncnn(self, prefix=colorstr("NCNN:")):
        ...
        ncnn_args = [
            f"ncnnparam={f / 'models.ncnn.param'}",
            f"ncnnbin={f / 'models.ncnn.bin'}",
            f"ncnnpy={f / 'model_ncnn.py'}",
        ]

        pnnx_args = [
            f"pnnxparam={f / 'models.pnnx.param'}",
            f"pnnxbin={f / 'models.pnnx.bin'}",
            f"pnnxpy={f / 'model_pnnx.py'}",
            f"pnnxonnx={f / 'models.pnnx.onnx'}",
        ]
    ...
    def export_coreml(self, prefix=colorstr("CoreML:")):
        ...
        if self.model.task == "classify":
            ...
            if self.args.nms:
                ...
                # TODO CoreML Segment and Pose models pipelining
            model = self.model

        ts = torch.jit.trace(model.eval(), self.im, strict=False)  # TorchScript models
        ct_model = ct.convert(
            ts,
    ...
    def export_engine(self, dla=None, prefix=colorstr("TensorRT:")):
        ...
        if self.args.dynamic:
            shape = self.im.shape
            if shape[0] <= 1:
                LOGGER.warning(f"{prefix} WARNING ⚠️ 'dynamic=True' models requires max batch size, i.e. 'batch=16'")
    ...
    def export_saved_model(self, prefix=colorstr("TensorFlow SavedModel:")):
        ...
        keras_model = onnx2tf.convert(
            ...
            disable_group_convolution=True,  # for end-to-end models compatibility
            enable_batchmatmul_unfold=True,  # for end-to-end models compatibility
        )
        yaml_save(f / "metadata.yaml", self.metadata)  # add metadata.yaml
    ...
    def export_pb(self, keras_model, prefix=colorstr("TensorFlow GraphDef:")):
        ...
        m = tf.function(lambda x: keras_model(x))  # full models
    ...
    def export_edgetpu(self, tflite_model="", prefix=colorstr("Edge TPU:")):
        ...
        f = str(tflite_model).replace(".tflite", "_edgetpu.tflite")  # Edge TPU models
    ...
    def export_tfjs(self, prefix=colorstr("TensorFlow.js:")):
        ...
        if " " in f:
            LOGGER.warning(f"{prefix} WARNING ⚠️ your models may not work correctly with spaces in path '{f}'.")
    ...
    def export_imx(self, prefix=colorstr("IMX:")):
        ...
        check_requirements(("models-compression-toolkit==2.1.1", "sony-custom-layers==0.2.0", "tensorflow==2.12.0"))
        ...
        model_onnx = onnx.load(onnx_model)  # load onnx models
        for k, v in self.metadata.items():
            meta = model_onnx.metadata_props.add()
            meta.key, meta.value = k, str(v)
    ...
    def _pipeline_coreml(self, model, weights_dir=None, prefix=colorstr("CoreML Pipeline:")):
        ...
        else:  # linux and windows can not run models.predict(), get sizes from PyTorch models output y
...
class IOSDetectModel(torch.nn.Module):
    """Wrap an Ultralytics YOLO models for Apple iOS CoreML export."""

    def __init__(self, model, im):
        """Initialize the IOSDetectModel class with a YOLO models and example image."""
        super().__init__()
    ...
    def forward(self, x):
        """Normalize predictions of object detection models with input size-dependent factors."""
...
class NMSModel(torch.nn.Module):
    ...
    def __init__(self, model, args):
        ...
        Args:
            model (torch.nn.module): The models to wrap with NMS postprocessing.
