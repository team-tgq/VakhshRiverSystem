...
class HillSWE:
    ...
    def predict(self, data: pd.DataFrame, pptwt: str, TD: str, DOY: str, snow_depth: str, DOY_ : int = 180) -> Union[np.ndarray, pd.Series]:
        ...
        # Calculate final SWE using the Hill models
...
class StatisticalModels(HillSWE):
    def __init__(self, algorithm: str = 'default', return_type='numpy'):
        """
        Initialize the SWE models with a specified algorithm and additional keyword arguments.
    ...
    def predict(self, 
        ...
        if self.algorithm.lower() == 'hill':
            ...
            try:
                depth = data[depth_col].to_numpy() if depth_col else data[kwargs.get('snow_depth')].to_numpy()
            except KeyError as e:
                raise ValueError(f"Missing required column: {e.args[0]}")
            
            if 'snow_depth' in kwargs:
                kwargs.pop('snow_depth')
