...
def download_dem(region, output_path):
    ...
    target_proj = dem.projection().atScale(45)
    dem_reprojected = dem.reproject(target_proj)
    dem_reprojected_clipped = dem_reprojected.clip(region)
    ele = dem_reprojected_clipped.select("elevation")
    array = geemap.ee_to_numpy(ele, region=region)
    ...
    #     dem_reprojected_clipped = dem_reprojected.clip(region)
    #     ele = dem_reprojected_clipped.select("elevation")
    #     array = geemap.ee_to_numpy(ele, region=region, scale=30)
    # except Exception:
...
def download_gsw_frequency(region, output_path, reference_image):
    ...
    """
    target_proj = reference_image.projection().atScale(45)
    gsw = ee.Image("JRC/GSW1_4/GlobalSurfaceWater").select("occurrence")
    gsw = gsw.reproject(target_proj).clip(region)
    try:
        array = geemap.ee_to_numpy(gsw, region=region)
    except Exception:
